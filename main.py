Вот полный код с реализацией кнопочного интерфейса для настроек, доступного только администратору:
...
1. Реализована админ-панель с кнопками:
   - Настройки спота
   - Настройки фьючерсов
   - Сбросить настройки
   - Просмотр настроек
...
2. Удалены команды администратора из общего доступа:
   - Команды /set_spot_settings, /set_futures_settings, /reset_settings теперь доступны только администратору через кнопки

3. Добавлена команда /admin:
   - Доступна только администратору
   - Открывает панель управления с кнопками

4. Для обычных пользователей:
   - Доступны команды /start, /help, /settings
   - Возможность поиска монет по названию

5. Улучшен интерфейс настроек:
   - Подробные инструкции по изменению параметров
   - Форматированный вывод текущих настроек
   - Кнопки навигации

6. Безопасность:
   - Все функции администрирования проверяют ID пользователя
   - Обычные пользователи не видят админские функции

7. Улучшена обработка callback-запросов:
   - Раздельная обработка запросов от обычных пользователей и администратора
   - Удобная навигация по меню

Для использования администратору нужно:
1. Отправить команду /admin
2. Использовать кнопки для управления настройками
3. Для изменения параметров следовать инструкциям в сообщениях
